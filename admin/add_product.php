    ...
    if (empty($name) || $price <= 0) {
        ...
        if (empty($error)) {
            ...
            if (mysqli_stmt_execute($s)) {
                $success = $edit_id ? "Product updated successfully!" : "Product added successfully!";
                if (!$edit_id) {
                    // Reset form
                    $product = null;
                    $edit_id = 0;
...
<style>
  .admin-wrap { max-width: 780px; margin: 0 auto; padding: 2.5rem 1.5rem 5rem; }
  ...
  .form-card {
    ...
    box-shadow: 0 2px 16px rgba(0,0,0,0.06); padding: 2rem;
  }

  .alert {
    padding: 0.75rem 1rem; border-radius: 8px; font-size: 0.85rem; margin-bottom: 1.2rem;
  }
  ...
  .form-group label {
    display: block; font-size: 0.76rem; font-weight: 700; color: var(--dark);
  ...
  .form-group textarea {
    ...
    border: 2px solid #e5e0f5; border-radius: 10px;
  ...
  .upload-area .icon { font-size: 2rem; margin-bottom: 0.4rem; }
    ...
    <form method="POST" action="" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <label class="upload-area" for="imgInput">
          ...
          <div class="icon">📷</div>
          <p><span>Click to upload</span> or drag & drop<br/>JPG, PNG, WEBP — max 5MB</p>
